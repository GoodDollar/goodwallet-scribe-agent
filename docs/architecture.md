...
6. **Assemble bounded context.** For each primary document, `collectPrimaryContext` (inside `context.ts`):
   ...
   - Link destinations come from `markdown-links.ts`, which strips optional titles, unwraps `<...>` destinations, percent-decodes paths, and ignores links inside fenced code blocks and inside matched inline code spans (any fence width, several spans per line); an unmatched backtick run stays literal so later links on the line still count.
...
12. **Deliver.** The CLI writes the report to stdout and exits `0`. The Action writes it to the job summary if `config.report.summary`, and if `config.report.comment` and the run has a PR number, upserts a sticky comment, capped at GitHub's 65,536-character body limit by dropping whole trailing lines and appending an explicit truncation notice (see [`security.md`](security.md) for the fork-token fallback and why the cut is line-aligned). Any thrown error at any step above sets the CLI's exit code to `1` (message on stderr) or calls `core.setFailed(message)` in the Action — content findings never do either.
