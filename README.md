...
The comment body (marker included) is capped at GitHub's 65,536-character limit before the API call. A report that would exceed it is truncated **at complete line boundaries only** and ends with an explicit notice — `The report was truncated because it exceeded GitHub's comment size limit.` — so an oversized report degrades to a truncated comment instead of an API rejection. Because the renderer keeps every finding value on one line, cutting only between lines guarantees no code span is left half-open; a line that will not fit whole is dropped entirely rather than sliced, so the marker, the header, and the notice always fit. The job summary is written unabridged.
...
- Link parsing handles common inline links line by line. Reference-style links (`[text][label]`), autolinks, and destinations split across lines are not resolved, so they are neither followed into context nor link-checked. Inline code spans are detected per line as well, so a span opened on one line and closed on another does not hide links from the checker.
- When a report is truncated for the PR comment, the dropped findings are only visible in the job summary; truncation cuts the tail rather than summarizing what was removed.
